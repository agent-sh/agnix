...
### Added
- **`agnix schema --fix` for source-controlled schema drift detection (#716)**. Pete source-controls `schemas/agnix.json` and wanted a pre-commit hook that regenerates it when the agnix binary is bumped. New `--fix` flag reads the current target file, compares against the binary's live schema output, and overwrites only when they differ. Defaults the target path to `schemas/agnix.json` when `--output` is absent. Silent on clean runs (pre-commit stays quiet on no-op), prints `Schema created: <path>` or `Schema updated: <path>` when it writes. Creates parent directories as needed. Schema output now ends with a trailing newline so on-disk diffs stay clean. `.pre-commit-hooks.yaml` ships a new `agnix-schema` hook entry that runs `agnix schema --fix` with `always_run: true` + `pass_filenames: false`. 5 new integration tests cover create-missing, silent-on-clean, update-on-drift, custom path with parent creation, and --help discoverability. Existing `agnix schema` and `agnix schema --output <path>` behavior preserved. Closes #716.
