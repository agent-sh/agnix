...
### Changed
- **Tool baseline**: `codex` bumped `rust-v0.136.0` -> `rust-v0.137.0` (closes #1013). Diffed upstream `codex-rs/core/config.schema.json` and refreshed Codex config allow-lists for new `local_thread_store_compression` / `unified_exec_zsh_fork` feature flags plus per-app `approvals_reviewer`, preventing false positives from `CDX-CFG-011` and `CDX-CFG-006`. Added `CDX-PL-015` for non-string `.codex-plugin/plugin.json` `skills` fields, which Codex now ignores with a warning. `.github/tool-release-baselines.json` and `knowledge-base/RESEARCH-TRACKING.md` updated.
