...
### Fixed
- **CC-HK-008 false-positive on `~`-prefixed hook paths** (closes #894, reported by @philipstancil). Claude Code passes hook commands straight to the shell, which expands a leading `~/` to `$HOME/...` at runtime. agnix was resolving `~/.claude/hooks/hook.py` relative to the project dir and reporting `Script file not found at '~/...' (resolved to '/cwd/~/.claude/hooks/hook.py')` on every hook using the canonical `~/` prefix - which is what every Claude Code docs example uses. `resolve_script_path` now expands a leading `~/` (and `~\\` on Windows) via `dirs::home_dir()` before the existence check, and `has_unresolved_env_vars` treats `~user/...` forms (arbitrary user home) as unresolved so CC-HK-008 skips them rather than false-positive. The existing `$CLAUDE_PROJECT_DIR` / `$FOO` semantics are preserved - verified by regression guards. Covered by 8 new unit tests in `rules::hooks::helpers::tests` plus 2 integration tests in `rules::hooks::tests`. Bug 2 from #894 (timeout units) was not a bug - the Claude Code binary's own Zod schema calls out `"Timeout in seconds for this specific command"` for all four hook types, so CC-HK-010's "exceeds 600s default" wording matches upstream semantics. Replied to that point on the issue. The `kiro.validators` array listed five validators (`kiro_steering`, `kiro_power`, `kiro_agent`, `kiro_hook`, `kiro_mcp`) but `crates/agnix-core/src/rules/kiro_settings.rs` (shipped in v0.21.0 with KR-SET-001/002/003) was absent. The array is consumed by `.github/workflows/tool-release-watch.yml` to classify changes of interest per tool; with `kiro_settings` missing, upstream changes to `.kiro/settings.json` parsing or schema would not be correctly attributed. Added `kiro_settings` to the array. Pairs with #879 (RESEARCH-TRACKING Kiro row drift).
