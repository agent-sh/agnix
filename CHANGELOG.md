...
- **Claude-specific skill/agent rules corrected and scoped (part of #957).**
  - **CC-SK-008** (unknown tool name): refreshed `KNOWN_TOOLS` to the current Claude Code built-in set (adds `PowerShell` - the reported false positive - plus `Agent`, `Cron*`, `Team*`, `EnterWorktree`/`ExitWorktree`, `ScheduleWakeup`, `ListMcpResourcesTool`/`ReadMcpResourceTool`/`WaitForMcpServers`, etc.; legacy names kept). Now **fires only for Claude Code skills** - other clients have their own tool vocabularies.
  - **CC-SK-017** (unknown frontmatter field): added the documented `when_to_use` and `arguments` fields. Now **fires only for Claude Code skills** (other clients' fields are checked by the per-client skill validator). `aliases` remains correctly flagged - it is not a documented Claude skill field.
