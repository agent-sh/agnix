...
**Detection**: Project-level check. Builds the chain the way Codex discovers it - start at the project root, walk down, at most one file per directory, preferring `AGENTS.override.md` over `AGENTS.md` - then sums file sizes root-first and reports on the file where the running total crosses the limit, since that file and everything deeper is what Codex drops. Conservative by design: `project_doc_fallback_filenames` lives in the user's Codex config rather than anything agnix reads, so a project using fallbacks has a longer real chain and this check under-reports rather than over-reports.
