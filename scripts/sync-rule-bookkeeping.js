 ...
 *    - `editors/vscode/README.md`, `crates/agnix-lsp/README.md`,
 *      `docs/EDITOR-SETUP.md`, `website/docs/{intro,getting-started,
 *      contributing}.md`
 *
 ...
 *
 * 4b. Per-category count tables in `knowledge-base/INDEX.md`,
 *    `knowledge-base/VALIDATION-RULES.md`, and `SPEC.md`, by shelling
 *    out to `scripts/check-rule-counts.py --fix`. That script owns the
 *    category->table-label maps, so it does the rewrite rather than
 *    this one duplicating them.
 *
...
const COUNT_FILES = [
  ...
  // Editor + website docs. scripts/check-rule-counts.py enforces the
  // total in all six of these but nothing wrote them, so every rule
  // addition failed the "Rule count table check" CI step until someone
  // hand-patched the numbers (found while adding OC-DEP-007). Each uses
  // plain "N rules" / "N validation rules" phrasing, so the default
  // patterns apply. website/docs/rules/index.md is excluded: it is
  // generated by scripts/generate-docs-rules.py in step 5.
  { path: path.join(ROOT, 'editors', 'vscode', 'README.md') },
  { path: path.join(ROOT, 'crates', 'agnix-lsp', 'README.md') },
  { path: path.join(ROOT, 'docs', 'EDITOR-SETUP.md') },
...
// ---- 4b. Per-category count tables ----
//
// knowledge-base/INDEX.md, knowledge-base/VALIDATION-RULES.md, and
// SPEC.md carry per-category Rules/HIGH/MEDIUM/LOW/Auto-Fix tables.
// scripts/check-rule-counts.py already owns the category->label maps
// that those rows key on, so rather than duplicate them here (and
// guarantee they drift apart) we shell out to it with --fix. In
// --check mode we run it read-only and surface any drift.

if (!skipDocs) {
  const args = [path.join(ROOT, 'scripts', 'check-rule-counts.py')];
  if (!checkMode) args.push('--fix');
  const pythonCandidates = ['python3', 'python'];
  let ran = false;
  let lastErr = null;
  for (const bin of pythonCandidates) {
    try {
      const output = execFileSync(bin, args, { cwd: ROOT, encoding: 'utf8' });
      if (output.trim()) {
        console.log(output.trim().split('\n').map((l) => `     ${l}`).join('\n'));
      }
      console.log('[OK] Rule count tables are in sync');
      ran = true;
      break;
    } catch (err) {
      // Exit 1 means real drift it could not fix (a missing row needs a
      // human to place it). ENOENT means this interpreter isn't present,
      // so fall through to the next candidate.
      if (err.status === 1) {
        const detail = `${err.stdout || ''}${err.stderr || ''}`.trim();
        note(
          `check-rule-counts.py reported count drift it could not fix:\n${detail}`
        );
        ran = true;
        break;
      }
  ...
  if (!ran) {
    console.error(
      '[WARN] Could not run check-rule-counts.py (python3/python unavailable?):',
      lastErr ? lastErr.message : 'unknown error'
    );
    // Not a hard failure - the CI "Rule count table check" step gates this.
  }
}
